...
import '../parsing/persisted_data_parser.dart';
...
class DailyPlan {
  ...
  /// Whether the sabqi (recent review) phase has any content today.
  bool get hasSabqiContent => sabqiPages.isNotEmpty;

  /// Whether the manzil (long-term review) phase has any content today.
  bool get hasManzilContent => manzilPages.isNotEmpty;

  /// Whether each phase needs no in-app work today.
  ///
  /// The generation-time `*DoneOffline` auto-skip is retired (roadmap
  /// Phase 1 task 7): generators never set the flags anymore. Empty phases
  /// are skipped through these derived checks instead, and the stored
  /// `*DoneOffline` flags carry exactly one meaning — the user marked the
  /// phase as completed outside the app (completion tracking, which stays
  /// until Phase 4 moves completion onto session facts).
  bool get isSabaqDone => sabaqDoneOffline;

  /// See [isSabaqDone] — done offline, or generated with no sabqi content.
  bool get isSabqiDone => sabqiDoneOffline || sabqiPages.isEmpty;

  /// See [isSabaqDone] — done offline, or generated with no manzil content.
  bool get isManzilDone => manzilDoneOffline || manzilPages.isEmpty;
